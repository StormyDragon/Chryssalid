...
var ENTRY_POINT =
    process.env.X_GOOGLE_ENTRY_POINT ? process.env.X_GOOGLE_ENTRY_POINT : 'function';
var SUPERVISOR_HOSTNAME =
    process.env.X_GOOGLE_SUPERVISOR_HOSTNAME ? process.env.X_GOOGLE_SUPERVISOR_HOSTNAME : null;
    ...
    try {
        ...
        if (typeof userFunction != 'function') {
            processUserCodeError(
                ...
                ' as ' + ENTRY_POINT + ' needs to be of type function. Got: ' +
                typeof userFunction);
            return null;
        }
...
var adjustUserReq = function (req) {
    req.url = removePrefix(req.url, EXECUTE_PREFIX);
    req.path = removePrefix(req.path, EXECUTE_PREFIX);
    req.baseUrl = removePrefix(req.baseUrl, EXECUTE_PREFIX);
    req.originalUrl = removePrefix(req.originalUrl, EXECUTE_PREFIX);
    if (req.route != null) {
        path = removePrefix(req.route.path, EXECUTE_PREFIX);
